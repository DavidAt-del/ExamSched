/**
 * Rich local demo seed.
 *
 * Usage:
 *   npm run seed:demo --workspace @app/api
 *   npx tsx scripts/seed-dev.ts
 *
 * This wipes the local dev DB and inserts a realistic slice of data so the
 * UI can be explored immediately:
 *   - admin + exam staff + 4 proctors
 *   - open / scheduled / sent / closed exam periods
 *   - open-period availability responses
 *   - scheduled + sent assignments
 *   - notification log rows
 *   - audit log trail for the admin page
 */
...
const addHours = (days: number, hours: number, minutes = 0): Date => {
  const d = new Date(NOW);
...
const OPEN_AVAILABILITY_SUBMISSION_IDS = [
...
const AVAILABILITY_SUBMISSIONS = [
  { id: OPEN_AVAILABILITY_SUBMISSION_IDS[0]!, userId: USERS[2]!.id, periodId: PERIODS.open.id, submittedAt: addHours(-1, 16, 30) },
  { id: OPEN_AVAILABILITY_SUBMISSION_IDS[1]!, userId: USERS[3]!.id, periodId: PERIODS.open.id, submittedAt: addHours(-1, 16, 45) },
  { id: OPEN_AVAILABILITY_SUBMISSION_IDS[2]!, userId: USERS[4]!.id, periodId: PERIODS.open.id, submittedAt: addHours(-1, 17, 0) },
  { id: OPEN_AVAILABILITY_SUBMISSION_IDS[3]!, userId: USERS[5]!.id, periodId: PERIODS.open.id, submittedAt: addHours(-1, 17, 15) },
];
